-- Alter the check constraint for market_items type
ALTER TABLE market_items DROP CONSTRAINT IF EXISTS valid_type;

ALTER TABLE market_items ADD CONSTRAINT valid_type
  CHECK (type IN ('food', 'training', 'boost', 'cosmetic', 'gold'));
