-- Drop and recreate buy_animal function with proper debugging
...
BEGIN
    ...
    -- Debug: Log the hunger rate we're about to use
    RAISE NOTICE 'Market animal hunger_rate: %', v_market_animal.hunger_rate;

    -- Validate hunger_rate exists
    ...
    -- Create the animal for the user with explicit hunger_rate
    INSERT INTO animals (
        user_id,
        name,
        type,
        ...
        run_anim,
        market_animal_id,
        hunger_rate,        -- Explicitly set this
        hunger_level,
        last_fed
    ) VALUES (
        auth.uid(),
        ...
        p_market_id,
        v_market_animal.hunger_rate,  -- This should copy the hunger rate
        100,
        CURRENT_TIMESTAMP
    );
    ...
    -- Debug: Log what we inserted
    RAISE NOTICE 'Inserted animal with hunger_rate: %', v_market_animal.hunger_rate;
END;
