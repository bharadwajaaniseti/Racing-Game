...
-- Drop old trigger functions that were forcing default values
...
-- Remove default value for hunger_rate if it exists
ALTER TABLE animals 
ALTER COLUMN hunger_rate DROP DEFAULT;

-- Fix all animals to use their market template's hunger rate
UPDATE animals a
SET hunger_rate = ma.hunger_rate
FROM market_animals ma
WHERE a.market_animal_id = ma.id;

-- Specifically force update wolf's hunger rate
UPDATE animals a
SET hunger_rate = ma.hunger_rate
FROM market_animals ma
WHERE a.market_animal_id = ma.id
